...
public: 
	...
	Game(int white, int pboard, int turn){
	...
	void update_board(){
		...
		if (!this->mated){
			...
			else if (this->boardnm.only_two_kings() || this->boardnm.stalemate()){

				this->boardsq.change_fill_color(sf::Color(30, 255, 30, 255), curr_king_pos.x, curr_king_pos.y);
				this->boardsq.change_fill_color(sf::Color(30, 255, 30, 255), this->boardnm.get_king(-this->boardnm.get_turn()).x, this->boardnm.get_king(-this->boardnm.get_turn()).y);
				this->end = true;
			}
		}
